...
class TransformerDetector:
    ...
    def _boost_confidence(self, text, detection):
        ...
        for word in self.context_words.get(detection["entity_type"], []):
            if word in context:
                detection["confidence"] = min(
                    detection["confidence"] + 0.15, 1.0
                )
    ...
    def detect(self, text: str):
        ...
        for r in results:
            ...
            detection = self._boost_confidence(text, detection)
            detections.append(detection)

        detections.sort(key=lambda x: (x["start"], -x["confidence"]))
